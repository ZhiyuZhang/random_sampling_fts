...
#   ax_f.plot(cent_spec_SSW[0], spec_n_sinc ,    label='SSW'   , linewidth=0.1 )
    ...
    plt.savefig('plots/overlay/'+str(FTSobsid)+str(index)+'_sinc_only.pdf')
...
#-------------- plot the baseline and original spectra ----------------
    ...
    textlines(ax_f,redshift,0.3,freq_list)
    plt.legend( loc=2, borderaxespad=0.)
    plt.savefig('plots/overlay/'+str(FTSobsid)+str(index)+'_subtracted.pdf')
#-------------- plot the baseline and original spectra ----------------
...
#   ax_f.plot(cent_spec_SSW[0], cent_SLW_err*(-1))
    ...
    textlines(ax_f,redshift,0.3,freq_list)
    plt.legend( loc=2, borderaxespad=0.)
    plt.savefig('plots/overlay/all'+str(FTSobsid)+str(index)+'_subtracted.pdf')

    ax_f.set_ylim(-0.5,0.7)
    plt.legend( loc=2, borderaxespad=0.)
    textlines(ax_f,redshift,0.3,freq_list)
    ax_f.set_xlim(freq_Cp -20, freq_Cp +20)
    plt.savefig('plots/overlay/cplus'+str(FTSobsid)+str(index)+'_subtracted.pdf')
    ax_f.set_xlim(freq_Oiii88-20, freq_Oiii88+20)
    plt.savefig('plots/overlay/oiii88'+str(FTSobsid)+str(index)+'_subtracted.pdf')
